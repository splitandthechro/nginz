...
vec4 calcPointLight (PointLight pointLight, vec3 normal, vec3 worldPos, float specularPower, float specularIntensity) {
  ...
  vec4 color = calcLight (pointLight.base, lightDirection, normal, specularPower, specularIntensity, worldPos);
...
void main () {
	...
	frag_color = vec4 (diffuseColor, 1.0) * calcPointLight (pointLight, normal, position, 2, 2);
}